...
import { appendToGoogleSheet, initializeGoogleSheet } from '@/utils/googleSheetsIntegration';
  ...
  try {
    ...
    if (data.submissionType === 'business_acquisition') {
      ...
      try {
        ...
        console.log('Sending business lead to webhook:', webhookData);
        ...
        if (!response.ok) {
          ...
          // Fallback to GHL if webhook fails
          const ghlSuccess = await ghlIntegration.submitBusinessLead(data);
          if (!ghlSuccess) {
            console.error('Failed to submit to both webhook and GHL');
          }
        } else {
          const result = await response.json().catch(() => ({}));
          console.log('Successfully sent business lead to webhook:', result);
        }
      } catch (error) {
      ...
      try {
        ...
        await initializeGoogleSheet(); // Ensure headers are set
        const googleSheetsSuccess = await appendToGoogleSheet(data);
        if (googleSheetsSuccess) {
          console.log('Successfully sent business lead to Google Sheets');
        } else {
      ...
      // Send to Google Sheets
      try {
        console.log('Sending property lead to Google Sheets...');
        await initializeGoogleSheet(); // Ensure headers are set
        const googleSheetsSuccess = await appendToGoogleSheet(data);
        if (googleSheetsSuccess) {
          console.log('Successfully sent property lead to Google Sheets');
        } else {
